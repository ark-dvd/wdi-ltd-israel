...
export default function ContentItemEditPage() {
  ...
  useEffect(() => {
    fetch(`/api/content-library/${params.id}`)
      .then(res => res.json())
  ...
  async function handleDelete() {
    ...
    try {
      await fetch(`/api/content-library/${params.id}`, { method: 'DELETE' });
  ...
  if (!item) return <div className="text-center py-12">פריט לא נמצא</div>;
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 border border-gray-300 rounded-lg"
            placeholder="מפרטים / רגולציה / מפרטים בינלאומיים"
          />
        </div>
