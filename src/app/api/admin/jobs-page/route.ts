/**
 * GET /api/admin/jobs-page — Load jobs page singleton
 * PUT /api/admin/jobs-page — Save jobs page singleton
 */
...
import { jobsPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "jobsPage"][0]`);
    if (!doc) return notFoundError('עמוד משרות לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const parsed = jobsPageUpdateSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const existing = await sanityClient.fetch(`*[_type == "jobsPage"][0]`);
    if (!existing) return notFoundError('עמוד משרות לא נמצא');
