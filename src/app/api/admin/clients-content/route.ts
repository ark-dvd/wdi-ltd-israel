/**
 * GET /api/admin/clients-content — List client content
 * POST /api/admin/clients-content — Create client content
 * DOC-040 §2.9.4
 */
...
import { clientContentCreateSchema } from '@/lib/validation/input-schemas';
...
export const GET = withAuth(async (request: NextRequest) => {
  try {
    ...
    const [data, total] = await Promise.all([
      sanityClient.fetch(`*[${filter}] | order(order asc) [${offset}...${offset + limit}]{ _id, _type, name, websiteUrl, isActive, order, updatedAt }`),
...
export const POST = withAuth(async (request: NextRequest, { session }: AuthContext) => {
  try {
    ...
    const parsed = clientContentCreateSchema.safeParse(body);
    if (!parsed.success) {
