/**
 * GET /api/admin/innovation-page — Load innovation page singleton
 * PUT /api/admin/innovation-page — Save innovation page singleton
 */
...
import { innovationPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "innovationPage"][0]`);
    if (!doc) return notFoundError('עמוד חדשנות לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const parsed = innovationPageUpdateSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const existing = await sanityClient.fetch(`*[_type == "innovationPage"][0]`);
    if (!existing) return notFoundError('עמוד חדשנות לא נמצא');
