/**
 * GET /api/admin/press-page — Load press page singleton
 * PUT /api/admin/press-page — Save press page singleton
 */
...
import { pressPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "pressPage"][0]`);
    if (!doc) return notFoundError('עמוד כתבו עלינו לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const parsed = pressPageUpdateSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const existing = await sanityClient.fetch(`*[_type == "pressPage"][0]`);
    if (!existing) return notFoundError('עמוד כתבו עלינו לא נמצא');
