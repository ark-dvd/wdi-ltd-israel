/**
 * POST /api/admin/clients-crm/:id/transition — Client status transition
 * DOC-040 §2.3, §5.2
 */
...
export const POST = withAuth(async (request: NextRequest, { params, session }: AuthContext<{ id: string }>) => {
  try {
    ...
    if (!isTransitionAllowed(CLIENT_TRANSITIONS, existing.status, parsed.data.targetStatus)) {
    ...
    const activity = addActivityToTransaction(tx, {
      ...
      entityId: id,
      type: 'status_change',
      description: `סטטוס לקוח ${existing.name} שונה מ-${existing.status} ל-${parsed.data.targetStatus}`,
