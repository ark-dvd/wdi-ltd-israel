/**
 * GET /api/admin/contact-page — Load contact page singleton
 * PUT /api/admin/contact-page — Save contact page singleton
 */
...
import { contactPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "contactPage"][0]`);
    if (!doc) return notFoundError('עמוד צור קשר לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const existing = await sanityClient.fetch(`*[_type == "contactPage"][0]`);
    if (!existing) return notFoundError('עמוד צור קשר לא נמצא');
