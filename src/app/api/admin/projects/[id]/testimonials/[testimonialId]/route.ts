/**
 * PUT /api/admin/projects/:projectId/testimonials/:testimonialId — Update testimonial
 * DELETE /api/admin/projects/:projectId/testimonials/:testimonialId — Delete testimonial
...
import { testimonialUpdateSchema } from '@/lib/validation/input-schemas';

export const PUT = withAuth(async (request: NextRequest, { params }: AuthContext<{ id: string; testimonialId: string }>) => {
  try {
    ...
    const doc = await sanityWriteClient.patch(testimonialId).set({ ...fields, updatedAt: new Date().toISOString() }).commit();
...
export const DELETE = withAuth(async (request: NextRequest, { params }: AuthContext<{ id: string; testimonialId: string }>) => {
  try {
    const { id: projectId, testimonialId } = params;
