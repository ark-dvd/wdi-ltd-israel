/**
 * GET /api/admin/services-page — Load services page singleton
 * PUT /api/admin/services-page — Save services page singleton
 */
...
import { servicesPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "servicesPage"][0]`);
    if (!doc) return notFoundError('עמוד שירותים לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const parsed = servicesPageUpdateSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const existing = await sanityClient.fetch(`*[_type == "servicesPage"][0]`);
    if (!existing) return notFoundError('עמוד שירותים לא נמצא');
