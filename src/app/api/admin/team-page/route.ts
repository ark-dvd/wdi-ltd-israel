/**
 * GET /api/admin/team-page — Load team page singleton
 * PUT /api/admin/team-page — Save team page singleton
 */
...
import { teamPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "teamPage"][0]`);
    if (!doc) return notFoundError('עמוד צוות לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const existing = await sanityClient.fetch(`*[_type == "teamPage"][0]`);
    if (!existing) return notFoundError('עמוד צוות לא נמצא');
