/**
 * GET /api/admin/about-page — Load about page singleton
 * PUT /api/admin/about-page — Save about page singleton
 */
...
import { aboutPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "aboutPage"][0]`);
    if (!doc) return notFoundError('עמוד אודות לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const existing = await sanityClient.fetch(`*[_type == "aboutPage"][0]`);
    if (!existing) return notFoundError('עמוד אודות לא נמצא');
