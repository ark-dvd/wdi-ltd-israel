/**
 * GET /api/admin/content-library-page — Load content library page singleton
 * PUT /api/admin/content-library-page — Save content library page singleton
 */
...
import { contentLibraryPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "contentLibraryPage"][0]`);
    if (!doc) return notFoundError('עמוד מאגר מידע לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const parsed = contentLibraryPageUpdateSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const existing = await sanityClient.fetch(`*[_type == "contentLibraryPage"][0]`);
    if (!existing) return notFoundError('עמוד מאגר מידע לא נמצא');
