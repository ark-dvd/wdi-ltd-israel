/**
 * GET /api/admin/content-library/:id — Fetch single content library item
 * PUT /api/admin/content-library/:id — Update content library item
 * DELETE /api/admin/content-library/:id — Delete content library item
 * DOC-040 §2.9.8
 */
...
import { contentLibraryUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async (_request: NextRequest, { params }: AuthContext<{ id: string }>) => {
  try {
    const { id } = params;
    const doc = await sanityClient.fetch(`*[_type == "contentLibraryItem" && _id == $id][0]`, { id });
...
export const PUT = withAuth(async (request: NextRequest, { params }: AuthContext<{ id: string }>) => {
  try {
    ...
    const parsed = contentLibraryUpdateSchema.safeParse(body);
    if (!parsed.success) {
