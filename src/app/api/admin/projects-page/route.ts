/**
 * GET /api/admin/projects-page — Load projects page singleton
 * PUT /api/admin/projects-page — Save projects page singleton
 */
...
import { projectsPageUpdateSchema } from '@/lib/validation/input-schemas';

export const GET = withAuth(async () => {
  try {
    const doc = await sanityClient.fetch(`*[_type == "projectsPage"][0]`);
    if (!doc) return notFoundError('עמוד פרויקטים לא נמצא');
...
export const PUT = withAuth(async (request: NextRequest) => {
  try {
    ...
    const parsed = projectsPageUpdateSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const existing = await sanityClient.fetch(`*[_type == "projectsPage"][0]`);
    if (!existing) return notFoundError('עמוד פרויקטים לא נמצא');
