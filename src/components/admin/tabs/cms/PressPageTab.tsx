...
export function PressPageTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try { const r = await apiGet<PageData>('/api/admin/press-page'); setData(r.data); setForm(r.data); setDirty(false); }
    catch (e) { setFetchErr(e as ErrorEnvelope); } finally { setLoading(false); }
  }, []);
  useEffect(() => { fetchData(); }, [fetchData]);

  const handleSave = async () => {
    const r = await execute(async () => apiPut<PageData>('/api/admin/press-page', { ...form, updatedAt: data?.updatedAt }));
    if (r) { const d = (r as { data: PageData }).data; setData(d); setForm(d); setDirty(false); addToast('עמוד כתבו עלינו נשמר', 'success'); }
  };

  const set = (key: string, val: unknown) => { setForm(p => ({ ...p, [key]: val })); setDirty(true); };

  if (loading) return <div className="p-8 text-center text-gray-500">טוען עמוד כתבו עלינו...</div>;
  ...
  return (
    <div className="p-6 max-w-2xl" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">עמוד כתבו עלינו</h1>
