...
interface PageData {
  ...
  categoryLabels: LabelPair[];
  updatedAt: string;
}

export function TeamPageTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try { const r = await apiGet<PageData>('/api/admin/team-page'); setData(r.data); setForm(r.data); setDirty(false); }
    catch (e) { setFetchErr(e as ErrorEnvelope); } finally { setLoading(false); }
  }, []);
  useEffect(() => { fetchData(); }, [fetchData]);

  const handleSave = async () => {
    const r = await execute(async () => apiPut<PageData>('/api/admin/team-page', { ...form, updatedAt: data?.updatedAt }));
    if (r) { const d = (r as { data: PageData }).data; setData(d); setForm(d); setDirty(false); addToast('עמוד צוות נשמר', 'success'); }
  };

  const set = (key: string, val: unknown) => { setForm(p => ({ ...p, [key]: val })); setDirty(true); };

  const addLabel = () => set('categoryLabels', [...((form.categoryLabels as LabelPair[]) || []), { _key: genKey(), value: '', label: '' }]);
  const updateLabel = (i: number, field: string, val: string) => {
    const arr = [...((form.categoryLabels as LabelPair[]) || [])]; arr[i] = { ...arr[i], [field]: val } as LabelPair; set('categoryLabels', arr);
  };
  const removeLabel = (i: number) => set('categoryLabels', ((form.categoryLabels as LabelPair[]) || []).filter((_, idx) => idx !== i));

  if (loading) return <div className="p-8 text-center text-gray-500">טוען עמוד צוות...</div>;
  ...
  return (
    <div className="p-6 max-w-2xl" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">עמוד צוות</h1>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-gray-800">תוויות קטגוריות</h2>
          ...
          {((form.categoryLabels as LabelPair[]) || []).map((s, i) => (
