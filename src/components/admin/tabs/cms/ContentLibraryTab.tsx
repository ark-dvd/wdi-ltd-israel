...
import { ContentLibraryPageTab } from './ContentLibraryPageTab';
...
interface ContentLibraryItem {
  ...
  fileUrl?: string; externalUrl?: string; isActive: boolean; order: number; updatedAt: string;
}
...
export function ContentLibraryTab() {
  ...
  const [form, setForm] = useState<Partial<ContentLibraryItem>>({ title: '', description: '', category: '', fileUrl: '', externalUrl: '', isActive: true, order: 0 });
  ...
  const fetchItems = useCallback(async () => {
    ...
    try { const r = await apiList<ContentLibraryItem>('/api/admin/content-library'); setItems(r.data); }
  ...
  const openCreate = () => { setEditId(null); setForm({ title: '', description: '', category: '', fileUrl: '', externalUrl: '', isActive: true, order: 0 }); reset(); setPanelOpen(true); };
  const openEdit = (i: ContentLibraryItem) => { setEditId(i._id); setForm({ ...i }); reset(); setPanelOpen(true); };

  const handleSave = async () => {
    const r = await execute(async () => {
      if (editId) return apiPut<ContentLibraryItem>(`/api/admin/content-library/${editId}`, { ...form, updatedAt: form.updatedAt });
      return apiPost<ContentLibraryItem>('/api/admin/content-library', form);
    });
    if (r) { const d = (r as { data: ContentLibraryItem }).data; setItems((p) => editId ? p.map((i) => i._id === editId ? d : i) : [...p, d]); setPanelOpen(false); addToast(editId ? 'פריט עודכן' : 'פריט נוצר', 'success'); }
  };
  ...
  if (showPageSettings) return (
    <>
      ...
      <ContentLibraryPageTab />
    </>
  );

  if (loading) return <div className="p-8 text-center text-gray-500">טוען מאגר מידע...</div>;
  ...
  return (
    <div className="p-6" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-6 mt-4">
        <h1 className="text-2xl font-bold text-gray-900">מאגר מידע</h1>
        <div className="flex items-center gap-3">
          <button onClick={fetchItems} className="p-2 text-gray-400 hover:text-gray-600" type="button"><RefreshCw size={18} /></button>
          <button onClick={openCreate} className="flex items-center gap-2 rounded-lg bg-wdi-primary px-4 py-2 text-sm font-medium text-white hover:bg-wdi-primary-light transition" type="button"><Plus size={16} />הוסף פריט</button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? <p className="text-gray-500 text-center py-12">אין פריטים במאגר</p> : (
        <div className="space-y-2">
          {filtered.map((c) => (
            <button key={c._id} onClick={() => openEdit(c)} className="w-full bg-white rounded-lg border border-gray-200 p-4 text-right hover:border-wdi-primary/30 hover:shadow-wdi-sm transition flex items-center justify-between" type="button">
              <div><p className="font-semibold text-gray-900">{c.title}</p><p className="text-sm text-gray-500">{c.category}</p></div>
              <span className={`text-xs px-2 py-0.5 rounded-full ${c.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{c.isActive ? 'פעיל' : 'מוסתר'}</span>
            </button>
          ))}
        </div>
      )}
      <SlidePanel open={panelOpen} title={editId ? 'עריכת פריט' : 'פריט חדש'} onClose={() => setPanelOpen(false)} footer={
        <div className="flex items-center gap-3">
        ...
        <div className="space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">תיאור</label><textarea value={form.description ?? ''} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} rows={3} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-wdi-primary focus:ring-1 focus:ring-wdi-primary" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">קטגוריה</label><input type="text" value={form.category ?? ''} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-wdi-primary focus:ring-1 focus:ring-wdi-primary" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">קישור חיצוני</label><input type="url" value={form.externalUrl ?? ''} onChange={(e) => setForm((p) => ({ ...p, externalUrl: e.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-wdi-primary focus:ring-1 focus:ring-wdi-primary" dir="ltr" /></div>
      ...
      <ConfirmDialog open={!!delConfirm} title="מחיקת פריט" message="פעולה זו תמחק את הרשומה לצמיתות. לא ניתן לבטל." confirmLabel="מחק" variant="danger" onConfirm={() => delConfirm && handleDelete(delConfirm)} onCancel={() => setDelConfirm(null)} />
    </div>
  );
}
