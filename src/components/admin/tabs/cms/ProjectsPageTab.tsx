...
export function ProjectsPageTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try { const r = await apiGet<PageData>('/api/admin/projects-page'); setData(r.data); setForm(r.data); setDirty(false); }
    catch (e) { setFetchErr(e as ErrorEnvelope); } finally { setLoading(false); }
  }, []);
  useEffect(() => { fetchData(); }, [fetchData]);

  const handleSave = async () => {
    const r = await execute(async () => apiPut<PageData>('/api/admin/projects-page', { ...form, updatedAt: data?.updatedAt }));
    if (r) { const d = (r as { data: PageData }).data; setData(d); setForm(d); setDirty(false); addToast('עמוד פרויקטים נשמר', 'success'); }
  };

  const set = (key: string, val: unknown) => { setForm(p => ({ ...p, [key]: val })); setDirty(true); };

  const addLabel = () => set('sectorLabels', [...((form.sectorLabels as LabelPair[]) || []), { _key: genKey(), value: '', label: '' }]);
  const updateLabel = (i: number, field: string, val: string) => {
    const arr = [...((form.sectorLabels as LabelPair[]) || [])]; arr[i] = { ...arr[i], [field]: val } as LabelPair; set('sectorLabels', arr);
  };
  const removeLabel = (i: number) => set('sectorLabels', ((form.sectorLabels as LabelPair[]) || []).filter((_, idx) => idx !== i));

  if (loading) return <div className="p-8 text-center text-gray-500">טוען עמוד פרויקטים...</div>;
  ...
  return (
    <div className="p-6 max-w-2xl" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">עמוד פרויקטים</h1>
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-gray-800">תוויות סקטורים</h2>
          ...
          {((form.sectorLabels as LabelPair[]) || []).map((s, i) => (
