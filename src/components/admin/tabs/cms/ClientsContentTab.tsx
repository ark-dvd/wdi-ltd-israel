...
interface ClientContent {
  _id: string; name: string; logo?: SanityImage | null; websiteUrl?: string; sector?: string; isActive: boolean; order: number; updatedAt: string;
}
type Filter = 'all' | 'active' | 'hidden';

export function ClientsContentTab() {
  ...
  const [form, setForm] = useState<Partial<ClientContent>>({ name: '', websiteUrl: '', isActive: true, order: 0 });
  ...
  const fetchItems = useCallback(async () => {
    ...
    try { const r = await apiList<ClientContent>('/api/admin/clients-content'); setItems(r.data); }
  ...
  const openCreate = () => { setEditId(null); setForm({ name: '', websiteUrl: '', isActive: true, order: 0 }); reset(); setPanelOpen(true); };
  const openEdit = (i: ClientContent) => { setEditId(i._id); setForm({ ...i }); reset(); setPanelOpen(true); };

  const handleSave = async () => {
    const r = await execute(async () => {
      if (editId) return apiPut<ClientContent>(`/api/admin/clients-content/${editId}`, { ...form, updatedAt: form.updatedAt });
      return apiPost<ClientContent>('/api/admin/clients-content', form);
    });
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">טוען לקוחות תוכן...</div>;
  if (fetchErr) return <div className="p-8"><ErrorRenderer error={fetchErr} onReload={fetchItems} /></div>;

  return (
    <div className="p-6" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">לקוחות (תוכן)</h1>
        <div className="flex items-center gap-3">
          <button onClick={fetchItems} className="p-2 text-gray-400 hover:text-gray-600" type="button"><RefreshCw size={18} /></button>
          <button onClick={openCreate} className="flex items-center gap-2 rounded-lg bg-wdi-primary px-4 py-2 text-sm font-medium text-white hover:bg-wdi-primary-light transition" type="button"><Plus size={16} />הוסף לקוח</button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? <p className="text-gray-500 text-center py-12">אין לקוחות תוכן</p> : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((c) => (
            <button key={c._id} onClick={() => openEdit(c)} className="bg-white rounded-lg border border-gray-200 p-4 text-center hover:border-wdi-primary/30 hover:shadow-wdi-sm transition" type="button">
              <p className="font-semibold text-gray-900">{c.name}</p>
              <span className={`text-xs px-2 py-0.5 rounded-full mt-2 inline-block ${c.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{c.isActive ? 'פעיל' : 'מוסתר'}</span>
            </button>
          ))}
        </div>
      )}
      <SlidePanel open={panelOpen} title={editId ? 'עריכת לקוח תוכן' : 'לקוח תוכן חדש'} onClose={() => setPanelOpen(false)} footer={
        <div className="flex items-center gap-3">
        ...
        <div className="space-y-4">
          <ImageUpload label="לוגו" value={form.logo ?? null} onChange={(v) => setForm((p) => ({ ...p, logo: v }))} />
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">אתר</label><input type="url" value={form.websiteUrl ?? ''} onChange={(e) => setForm((p) => ({ ...p, websiteUrl: e.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-wdi-primary focus:ring-1 focus:ring-wdi-primary" dir="ltr" /></div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">ענף</label><input type="text" value={form.sector ?? ''} onChange={(e) => setForm((p) => ({ ...p, sector: e.target.value }))} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-wdi-primary focus:ring-1 focus:ring-wdi-primary" /></div>
      ...
      <ConfirmDialog open={!!delConfirm} title="מחיקת לקוח תוכן" message="פעולה זו תמחק את הרשומה לצמיתות. לא ניתן לבטל." confirmLabel="מחק" variant="danger" onConfirm={() => delConfirm && handleDelete(delConfirm)} onCancel={() => setDelConfirm(null)} />
    </div>
  );
}
