...
export function ServicesPageTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try { const r = await apiGet<PageData>('/api/admin/services-page'); setData(r.data); setForm(r.data); setDirty(false); }
    catch (e) { setFetchErr(e as ErrorEnvelope); } finally { setLoading(false); }
  }, []);
  useEffect(() => { fetchData(); }, [fetchData]);

  const handleSave = async () => {
    const r = await execute(async () => apiPut<PageData>('/api/admin/services-page', { ...form, updatedAt: data?.updatedAt }));
    if (r) { const d = (r as { data: PageData }).data; setData(d); setForm(d); setDirty(false); addToast('עמוד שירותים נשמר', 'success'); }
  };

  const set = (key: string, val: unknown) => { setForm(p => ({ ...p, [key]: val })); setDirty(true); };

  if (loading) return <div className="p-8 text-center text-gray-500">טוען עמוד שירותים...</div>;
  ...
  return (
    <div className="p-6 max-w-2xl" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">עמוד שירותים</h1>
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">טקסט קרא עוד</label>
            <input type="text" value={form.readMoreText ?? ''} onChange={e => set('readMoreText', e.target.value)} className={inputCls} /></div>
        </div>
        <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
